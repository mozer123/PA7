...
public class MNIST {
     ...
     */
    public static float totalDist(float[] img1, float[] img2)
    ...
    public static int predict(float[] image, int k) {
        ...
        // compute priority and add to the priorityQueue
        for (int i = 0; i < NUM_TRAIN; i++) {
            float priority = totalDist(image, TRAIN_IMAGES[i]);
            pq.offer(new ImageLabel(TRAIN_LABELS[i], priority));
        }

        // create a counting array to store the frequency of each label
        int[] classCount = new int[NUM_CLASSES];

        // increment the count for each class label
        for (int i = 0; i < k; i++) {
            ImageLabel neighbour = pq.poll();
            classCount[neighbour.label]++;
        }

        // find the most popular class label
        int maxCount = -1;
        int predictLabel = -1;
